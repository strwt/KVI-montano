...
function Dashboard() {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <section className="relative overflow-hidden rounded-2xl border border-red-600 bg-gradient-to-br from-white to-neutral-100 p-6 text-neutral-900 shadow-[0_12px_30px_rgba(0,0,0,0.08)] dark:border-red-600 dark:bg-gradient-to-br dark:from-black dark:to-neutral-900 dark:text-white dark:shadow-[0_12px_30px_rgba(0,0,0,0.35)]">
        ...
        <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            <p className="text-[14px] uppercase tracking-[0.12em] text-neutral-500 dark:text-neutral-300">Volunteer Management</p>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <span className="rounded-lg border border-neutral-300 bg-white px-3 py-1 text-[14px] text-neutral-700 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white">{eventsThisMonth} this month</span>
              <span className="rounded-lg border border-neutral-300 bg-white px-3 py-1 text-[14px] text-neutral-700 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white">{categoriesUsed} active categories</span>
              <span className="rounded-lg border border-red-700/60 bg-red-600/15 px-3 py-1 text-[14px]">{isAdmin ? 'Administrator' : 'Member'}</span>
            </div>
          </div>
          <button
            type="button"
      ...
      <section className="grid grid-cols-12 gap-4">
        <article className="col-span-12 rounded-2xl border border-red-600 bg-white p-6 text-neutral-900 shadow-[0_10px_20px_rgba(0,0,0,0.08)] transition-all duration-200 hover:scale-[1.02] md:col-span-6 dark:border-red-600 dark:bg-black dark:text-white dark:shadow-[0_12px_24px_rgba(0,0,0,0.25)]">
          ...
          <p className="mt-2 text-[18px] text-neutral-900 dark:text-white">Total Events</p>
        </article>

        <article className="col-span-12 rounded-2xl border border-red-600 bg-white p-6 shadow-[0_10px_20px_rgba(0,0,0,0.08)] transition-all duration-200 hover:scale-[1.02] sm:col-span-6 md:col-span-3 dark:border-red-600 dark:bg-zinc-900">
          <p className="text-[14px] uppercase tracking-[0.12em] text-neutral-500 dark:text-zinc-400">This Month</p>
          <p className="mt-2 text-[32px] font-semibold text-black dark:text-zinc-100">{eventsThisMonth}</p>
        ...
        <article className="col-span-12 rounded-2xl border border-red-600 bg-white p-6 shadow-[0_10px_20px_rgba(0,0,0,0.08)] transition-all duration-200 hover:scale-[1.02] sm:col-span-6 md:col-span-3 dark:border-red-600 dark:bg-zinc-900">
          ...
          <p className="mt-2 text-[32px] font-semibold text-black dark:text-zinc-100">{recentEvents.length}</p>
          <p className="mt-2 text-[14px] text-neutral-500 dark:text-zinc-400">latest records</p>
        </article>
      </section>

      <section className="grid grid-cols-12 gap-4">
        <article className="col-span-12 rounded-2xl border border-red-600 bg-white p-6 shadow-[0_10px_20px_rgba(0,0,0,0.08)] md:col-span-7 dark:border-red-600 dark:bg-zinc-900">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-[24px] font-semibold text-black dark:text-zinc-100">Recent Activity</h2>
            <button
              type="button"
              onClick={() => navigate('/calendar')}
              className="cursor-pointer rounded-lg border border-red-600 bg-white px-4 py-2 text-[14px] font-medium text-red-600 transition-all duration-200 hover:scale-[1.02] hover:bg-red-600/10 dark:bg-zinc-900 dark:text-red-400"
            >
              View All
            </button>
          </div>
          <div className="space-y-4">
            {recentEvents.map((event, index) => (
              <button
                ...
                className={`group flex w-full cursor-pointer items-start gap-4 rounded-xl border border-red-600 bg-white p-4 text-left transition-all duration-200 hover:scale-[1.02] hover:shadow-[0_10px_16px_rgba(0,0,0,0.08)] dark:border-red-600 dark:bg-zinc-900 ${
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[18px] font-medium text-black dark:text-zinc-100">{event.title || 'Untitled Event'}</p>
        ...
        <article className="col-span-12 rounded-2xl border border-red-600 bg-white p-6 shadow-[0_10px_20px_rgba(0,0,0,0.08)] md:col-span-5 dark:border-red-600 dark:bg-zinc-900">
          <h2 className="mb-4 text-[24px] font-semibold text-black dark:text-zinc-100">Events by Category</h2>
          <div className="grid grid-cols-2 gap-2">
            {EVENT_CATEGORIES.map((category, index) => (
              <button
                ...
                className={`cursor-pointer rounded-xl border border-red-600 bg-white p-4 text-left transition-all duration-200 hover:scale-[1.02] hover:border-red-700 hover:shadow-[0_8px_14px_rgba(0,0,0,0.08)] dark:border-red-600 dark:bg-zinc-900 ${
                  animatedStats ? 'animate-fade-in-up' : 'opacity-0'
      ...
      <section className="grid grid-cols-12 gap-4">
        <article className="col-span-12 rounded-2xl border border-red-600 bg-white p-6 shadow-[0_10px_20px_rgba(0,0,0,0.08)] md:col-span-6 dark:border-red-600 dark:bg-zinc-900">
          <h2 className="mb-4 text-[24px] font-semibold text-black dark:text-zinc-100">Monthly Events</h2>
          <div className="overflow-x-auto rounded-xl border border-red-600 bg-neutral-50 p-4 dark:border-red-600 dark:bg-zinc-950">
              ...
              {monthlyEvents.map((item, index) => {
                ...
                return (
                  <g key={item.label}>
                    ...
                    <text x={x} y="214" textAnchor="middle" fontSize="12" fill="#525252" className="dashboard-chart-label">{item.label}</text>
                  </g>
                )
              })}
            </svg>
        ...
        <article className="col-span-12 rounded-2xl border border-red-600 bg-white p-6 shadow-[0_10px_20px_rgba(0,0,0,0.08)] md:col-span-3 dark:border-red-600 dark:bg-zinc-900">
          <h2 className="mb-4 text-[24px] font-semibold text-black dark:text-zinc-100">Category Share</h2>
      ...
      <section className="grid grid-cols-12 items-stretch gap-4">
        ...
        <article className="col-span-12 rounded-2xl border border-red-600 bg-white p-6 shadow-[0_10px_20px_rgba(0,0,0,0.08)] md:col-span-3 dark:border-red-600 dark:bg-zinc-900">
          <h2 className="mb-4 text-[24px] font-semibold text-black dark:text-zinc-100">Volunteer Participation</h2>
          <div className="space-y-4">
              ...
              return (
                <div key={`${item.name}-${index}`} className="space-y-1">
                  <div className="flex items-center justify-between text-[14px]">
                    ...
                    <span className="font-medium text-black dark:text-zinc-100">{item.count}</span>
                  </div>
