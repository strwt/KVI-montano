...
export default async function handler(req, res) {
  try {
    ...
    const achievementId = String(req.headers['x-achievement-id'] || '').trim()
    if (!achievementId) return res.status(400).json({ message: 'Missing x-achievement-id header.' })
    ...
    const path = `achievements/${achievementId}/${filename}`

    const { error: uploadError } = await supabaseAdmin.storage.from('achievement-images').upload(path, buffer, {
      upsert: true,
      contentType,
      cacheControl: '3600',
    })
    ...
    console.error('Unhandled error in /api/storage/upload-achievement-image.', error)
